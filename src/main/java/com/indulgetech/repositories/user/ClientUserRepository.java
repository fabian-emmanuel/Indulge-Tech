...
public interface ClientUserRepository extends JpaRepository<ClientUser, Long> {
    ...
    @Query("select u from ClientUser u " +
            "where u.email =:email " +
            "and u.auditSession.delF='0'")
    Optional<ClientUser> findByEmail(String email);

    @Query("select u from ClientUser u " +
            "where u.email =:username and u.auditSession.delF='0'")
    ...
    @Query("select new com.indulgetech.dto.user.ListUserDto(u.id,CONCAT(u.lastName,' ', u.firstName),u.email,u.telephoneNumber,u.status,u.auditSession.dateCreated) from ClientUser u WHERE u.auditSession.delF <> '1' order by u.lastName asc")
