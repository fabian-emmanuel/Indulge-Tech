...
class AdminUserTest extends AbstractTest {
    ...
    void shouldCreateAdminUserSuccessfully() throws Exception {
        ...
        MvcResult result = this.mockMvc
                .perform(post(path).content(TestUtils.asJsonString(dto))
    ...
    void shouldUpdateAdminUserSuccessfully() throws Exception {
        ...
        mockMvc.perform(put(path + "/{userId}", user.getId()).headers(headers).contentType("application/json")
        ...
        assertEquals(CustomDateUtils.today(), CustomDateUtils.getDateFromDateTime(updatedUser.getStatusDate()));
        ...
        assertTrue(updatedUser.getRoles().stream().map(Role::getId).toList().containsAll(dto.getRoles()));
        txnManager.endTxn();
    }
    ...
    void shouldDeactivateAdminUserAccountSuccessfully() throws Exception {
        ...
        this.userRepository.save(user);
        //act
        mockMvc.perform(put(path + "/{userId}/deactivate", user.getId()).headers(headers)).andExpect(status().isOk())
    ...
    void shouldActivateAdminUserAccountSuccessfully() throws Exception {
        ...
        this.userRepository.save(user);
        //act
        mockMvc.perform(put(path + "/{userId}/activate", user.getId()).headers(headers)).andExpect(status().isOk())
                .andExpect(jsonPath("$.message", Is.is("Resource deactivated successfully")));
        ...
        AdminUser loggedInUser = this.userRepository.findByEmail("admin@gmail.com").get();
        assertEquals(loggedInUser.getId(), activatedUser.getAuditSection().getModifiedBy());
    }
